...
export default function Navbar() {
  ...
  const handleClick = () => {
    if(localStorage.getItem('check')==='false'){
      ...
      // document.getElementsByClassName('createPage').style.pointerEvents = 'visible';
      window.open('/create','_self');
    }
  }
  return (
    <div>
      <MDBNavbar expand='lg' light style={{backgroundColor:'transparent'}}>
        <MDBContainer fluid className='nav-container'>
          ...
          <MDBCollapse navbar show={showNavNoToggler}>
            <MDBNavbarBrand href='#' className='ecommerce'>Blogify</MDBNavbarBrand>
            <MDBNavbarNav className='mr-auto mb-2 mb-lg-0 navLinks' >
              ...
              <MDBNavbarItem >
                <MDBNavbarLink active aria-current='page' href='#'>       
                  <Link className='link createPage' onClick={handleClick}>Create Blog</Link>
                </MDBNavbarLink>
              </MDBNavbarItem>
            </MDBNavbarNav>
